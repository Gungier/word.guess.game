...
var wordSpace = 0 ;             //amount of blank tiles word will take
var rightANDleft = [];           //Variable to hold the right answers and the remaining blank tiles
...
function begin (){
    ...
    compChoose = compList [Math.floor(Math.random () * compList.length)];  //select random word
    
    lettersINchoice = compChoose.split("");  //should break word into individual letter array

    wordSpace = lettersINchoice.length;  //should count the letters from last array

    rightANDleft = [];  //look at wordSpace and alter it based on guesses

    userWrong = []; //

    for (var x = 0; x < wordSpace; x++) {
        rightANDleft.push("_");  //please create the correct number of blank spaces from the lettersINchoice array *\./*

    }
console.log(compChoose);
console.log(rightANDleft);

document.getElementById("blankTiles").innerHTML = rightANDleft.join(" ");
...
function inspector (letter) { 
 ...
 if (inWord) {  //this figures out the WHERE of the letters especially with the variable rightANDleft

    for (var y = 0; y < wordSpace; y++) {

        if (compChoose[y] === letter) {

            rightANDleft[y] = letter;
        }
    }
...
function roundOver() {
    ...
    document.getElementById("blankTiles").innerHTML = rightANDleft; //appends the array 

    document.getElementById("wrongGuess").innerHTML = userWrong; //so the user can see what letters they've tried

if (lettersINchoice.toString() === rightANDleft.toString()) {
